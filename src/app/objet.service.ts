...
export class ObjetService {
  ...
  delete(objet: Objet) : void {
    this.http.delete(this.url + objet.id)
      .subscribe(data => {
        ...
        this.http.get<Objet[]>(this.url + '?idParent=' + objet.id).subscribe(
          (data : Objet[]) =>
          data.forEach(
            (item:Objet) =>
            {
  ...
  get(id?:string): Observable<Objet>{
    ...
    return new Observable(od => od.next({id:'',name:'First DIR',type:'dossier',last_modification:0,idParent:''}))
  }
